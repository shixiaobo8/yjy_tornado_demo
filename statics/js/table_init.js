...
$(document).ready(function () {
    ...
    $("#add").click(function () {
        ...
        if ($(this).find("i").hasClass("adding")) {//提交新建数据
            ...
            $.post(baseUrl + "interFeildAdd", {"key": key, "value": value}, function (data) {
                if (data['code'] == 200) {
                    ...
                    $table.bootstrapTable('destroy');//表格销毁
                    oTable.Init();
                    //$remind.html(' <i class="icon icon-info-sign">新建接口数据成功!.</i>');
                    alert('新建接口数据成功!');
                    location.reload();

                } else if (data['code'] == 202) {
                    $remind.html(' <i class="icon icon-info-sign">' + data['data'] + '</i>');
                    alert(data['data']);
                } else {
